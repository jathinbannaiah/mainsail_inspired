...
class GUI(QMainWindow, QObject):
    ...
    def __init__(self, parent_conn, child_conn, scanner_parent_conn, scanner_child_conn):
        try:
            ...
            self.read_thread = threading.Thread(target=self.serial.readData, args=(child_conn,self.scanner_parent_conn))
            ...
            self.set_attibutes.released.connect(self.R_motion_attributes)
            ...
            self.Zaxis_up.released.connect(lambda: self.Z_instance.Zaxis_up)
            self.Zaxis_down.released.connect(lambda: self.Z_instance.Zaxis_down)
    ...
    def start_process_function(self):  # Function to handle the automation loop
        try:
            ...
            self.waitforResponse()                        #when recoater go right is sent the tiva is put in a loop
    ...
    def start(self, motor_select):  # Function to handle the start of the motors
        try:
            ...
            if motor_select:  # Motor 1
                ...
                if self.serial.serial.is_open:
                    if MOTOR_DIR:
                        cmd = "Recoater_RUN_FW\r"      #Normal recoater motion speed
                        print(f"Command:{cmd}")
                        if self.debug_var == 2:
                            self.printToConsole(f"Command:{cmd}")
                            self.printToConsole(f"Linear right motion")
                        self.serial.serial_write(cmd)
                        self.recoater_normal_motion_direction = False
                        self.changeDIR.setText("Move left")
                    else:
                        cmd = "Recoater_RUN_RV\r"    #Recoater_START\r
                        print(f"Command:{cmd}")
                        if self.debug_var == 2:
                            self.printToConsole(f"Command:{cmd}")
                            self.printToConsole(f"Linear left motion")
                        self.serial.serial_write(cmd)
                        self.recoater_normal_motion_direction = True
                    ...
                    try:
                        if MOTOR_DIR:  # Variable to store the direction of motion
                            ...
                            if self.debug_var == 2:
                                if auto != "auto":
                                    self.printToConsole(f"Command:{cmd}")
                                    self.printToConsole(f"Non linear right motion")
                                print(f"Command:{cmd}")
                            self.serial.serial_write(cmd)
                            MOTOR_DIR = False
                            if auto != "auto":
                            ...
                            if self.debug_var == 2:
                                if auto != "auto":
                                    self.printToConsole(f"Command:{cmd}")
                                    self.printToConsole(f"Non linear left motion")
                                print(f"Command:{cmd}")
                            MOTOR_DIR = True
                            if auto != "auto":
                                self.changeDIR.setText("Move right")
